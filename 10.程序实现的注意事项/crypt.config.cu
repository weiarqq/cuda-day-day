    ...
    int plainLength, int nThreadsPerBlock)
{
    ...
    finishes = new cudaEvent_t[nBlocks];
    streams = new cudaStream_t[nBlocks];
    ...
    CHECK(cudaMemcpyToSymbolAsync(dkey, key, KEY_LENGTH * sizeof(int), 0, cudaMemcpyHostToDevice, streams[0]));
