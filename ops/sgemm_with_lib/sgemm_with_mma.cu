    ...
    const int K)
{
    ...
    // 一个block每个线程需要搬运多少次数据，一次能加载4个元素
    ...
    // 在这个维度为bm×bk的数据块中，当前线程需要搬运的数据的竖向坐标
    const int A_TILE_ROW_START = tid / A_TILE_THREAD_PER_ROW;
    const int B_TILE_ROW_START = tid / B_TILE_THREAD_PER_ROW;
    // 需要搬运的数据的横向坐标
    const int A_TILE_COL = tid % A_TILE_THREAD_PER_ROW * 4; // 横向一次搬运 4个元素
...
#pragma unroll
        ...
        // 小迭代 即rm*rn也需要预取，最后一次预取的还没算，现在算一下
