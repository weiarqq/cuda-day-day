...
// v0中，同一warp的线程，符合tid % (2 * stride) == 0的线程才进行计算，会造成线程束分化
// 我们可以换个思路，将符合 tid % (2 * stride)==0的线程需要读取的元素，映射为同一线程束连续的线程进行读取
// 即通过 同一线程束连续的线程id（tid）, 映射到对应的元素下标 index = tid * 2 * stride
__global__ void reduce(float* g_idata, float* g_odata, unsigned int n)
{
    ...
    for (auto stride = 1; stride < blockDim.x; stride <<= 1) {
        unsigned index = tid * 2 * stride;
        if (index < blockDim.x)
