...
// stride 是 2的幂，可以尝试将 for (auto stride = blockDim.x / 2; stride > 32; stride >>= 1) 完全展开
...
__global__ void reduce(float* g_idata, float* g_odata, unsigned int n)
{
    ...
    if (idx >= n)
        return;
    smem[tid] = g_idata[idx] + g_idata[idx + blockDim.x];
    __syncthreads();
    // BLOCK_SIZE不能超过 1024
    if (BLOCK_SIZE >= 512) {
        if (tid < 256) {
...
int main(int argc, char** argv)
{
    ...
    for (int k = 0; k < 10; k++) {
        ...
        iStart = cpuSecond();
        reduce<DIM><<<grid, block>>>(d_idata, d_odata, size);
