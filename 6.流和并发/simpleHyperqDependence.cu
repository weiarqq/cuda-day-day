...
int main(int argc, char** argv)
{
    ...
    cudaEvent_t* kernelEvent = new cudaEvent_t[n_streams];
    for (int i = 0; i < n_streams; i++) {
    ...
    /*
    实现最后一个流等其他流都完成后，再运行
    cudaStreamWaitEvent(streams[n_streams - 1], kernelEvent[i], 0); 指定多个kernelEvent[i]，等待其完成
    */
    ...
    for (int i = 0; i < n_streams; i++) {
        ...
        // cudaStreamWaitEvent使最后一个流（即streams[n_streams-1]​）等待其他所有流：
