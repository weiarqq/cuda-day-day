...
__global__ void gpuRecursiveReduce2(int* g_idata, int* g_odata,
    unsigned int istride, int const iDim)
{
    // convert global data pointer to the local pointer of this block
...
int main(int argc, char** argv)
{
    ...
    printf("gpuRecursiveReduceNosync\t\telapsed %f sec gpu_sum: %d <<<grid %d block %d>>>\n",
        iElaps, gpu_sum, grid.x, block.x);

    // gpu nested reduce kernel
    CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
    iStart = cpuSecond();
    gpuRecursiveReduce2<<<grid, block>>>(d_idata, d_odata, block.x >> 1, block.x);
    CHECK(cudaDeviceSynchronize());
    CHECK(cudaGetLastError());
    iElaps = cpuSecond() - iStart;
    CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int),
    ...
    printf("gpuRecursiveReduce2\t\telapsed %f sec gpu_sum: %d <<<grid %d block %d>>>\n",
        iElaps, gpu_sum, grid.x, block.x);
