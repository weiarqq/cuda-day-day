...
__global__ void setRowReadColDynamic(int* out)
{
    ...
    __syncthreads();
    out[row_idx] = tile[col_idx]; // 全局内存的存储操作 共享内存的加载操作
}
...
__global__ void setRowReadColDynPad(int* out)
{
    ...
    unsigned int col_idx = threadIdx.x * (blockDim.y + IPAD) + threadIdx.y; // threadIdx.x threadIdx.y坐标互换
    unsigned int idx = threadIdx.y * blockDim.x + threadIdx.x;

    tile[row_idx] = idx; // 共享内存的存储操作
