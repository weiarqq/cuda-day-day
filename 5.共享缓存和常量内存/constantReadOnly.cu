...
void initialDatalocal(float* in, const int size)
{
...
void printDatalocal(float* in, const int size)
{
...
int main(int argc, char** argv)
{
    ...
    size_t nBytes = (isize + 2 * RADIUS) * sizeof(float); // 多前后RADIUS的数据
    ...
    initialDatalocal(h_in, isize + 2 * RADIUS);
    ...
    // print out results
    if (iprint) {
        printDatalocal(gpuRef, isize);
        printDatalocal(hostRef, isize);
    }
